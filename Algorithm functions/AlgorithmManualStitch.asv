...
sy = ROIList(2)-ROIList(1)+1;
sx = ROIList(4)-ROIList(3)+1;
...
initPhasePupil = options.InitPhasePupil;
...
img_center(1) = (ROIList(2)-center(1)+ROIList(4)/2)*pixSizeObj/1000;
img_center(2) = (ROIList(1)-center(2)+ROIList(3)/2)*pixSizeObj/1000;
...
if ~isempty(svdIdx)
    idx_Y = round(svdIdx.idx_Y.*ROIList(4)./svdIdx.ROI(4));
    idx_X = round(svdIdx.idx_X.*ROIList(3)./svdIdx.ROI(3));
...
N_objX = ceil(N_objX2/ROIList(3))*ROIList(3);
if N_objX == ROIList(3)
    N_objX = N_objX*2;
end
N_objY = ROIList(4)*N_objX/ROIList(3);

parfor(i=1:nz)
    coords = ROIList(i);

    I = ImagesIn(coords(1):coords(2),coords(3):coords(4),:);

    %then crop
    %[ImagesIn,PhaseIn,imageColOrder] = InputImagesCrop(ImagesIn,PhaseIn,imageColOrder,LEDsUsed,ROI);
    [ImagesInCrop,imageColOrderCrop] = InputImagesCrop(ImagesIn,imageColOrder,LEDsUsed,ROI_List(i));
    ...
    [rec_object,phase,rec_pupil, err, erro,idx_X2,idx_Y2] = ... 
        ...
        recOrder, pupil0, options, cImag, f, false, rectype); %showIm=false
    %% convert rec_object to uint16
    rec_object = rec_object - min(min(rec_object)0
    
    imwrite(amplitude,[saveDir '\' num2str(i) '.tif'])
end
